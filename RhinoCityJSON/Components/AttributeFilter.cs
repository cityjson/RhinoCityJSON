...
{
    public class AttributeFilter : GH_Component
    {
        ...
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            ...
            pManager.AddGenericParameter("Filter Key", "Fk", "key to filter", GH_ParamAccess.list);
            pManager.AddTextParameter("Filter Value", "Fv", "Value to filter", GH_ParamAccess.list);
        ...
        {
            pManager.AddGenericParameter("Filtered information Objects", "Fi", "Filtered information Objects", GH_ParamAccess.item);
        }
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            DA.GetDataList(2, filterValueList);
            ...
            {
                AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, "No filter values found");
            }
            if (values.Count < 0)
            {
                return;
            ...
            if (!updatedList && filterValueList.Count > 0)
            {
                // filter process
                List<Types.GHObjectInfo> outputValues = new List<Types.GHObjectInfo>();
                for (int i = 0; i < values.Count(); i++)
                {
                    bool found = false;
                    var nPath = new Grasshopper.Kernel.Data.GH_Path(i);
                    List<string> answerList = values[i].Value.getItemByName(filterKey[0]);

                    foreach (var filterValue in filterValueList)
                    {
                        foreach (var item in answerList)
                        {
                            if (filterValue == item || filterValue + DefaultValues.defaultInheritanceAddition == item)
                            {
                ...
                {
                    AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, "No filtered output");
                    return;
                }
        ...
        {
            get { return new Guid("b2364c9a-18ae-4eb3-efb3-f76e8a274e45"); }
        }
    }
